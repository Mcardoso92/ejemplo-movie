        ...
        public async Task<IActionResult> Registro(RegistroViewModel usuario)
        {
            if (ModelState.IsValid)
            {
                ...
                var nuevoUsuario = new Usuario
                {
                    ...
                    ImagenUrlPerfil = "/images/default-avatar.png" // Puedes asignar una imagen predeterminada o permitir que el usuario la suba
                };
                ...
                if (resultado.Succeeded)
                {
                    ...
                    await _emailService.SendAsync(nuevoUsuario.Email, "Bienvenido a WhatMovie", "<h1>Gracias por registrarte en WhatMovie!</h1><p>Esperamos que disfrutes de nuestra plataforma.</p>");
                    return RedirectToAction("Index", "Home");
                }
                else
                {
